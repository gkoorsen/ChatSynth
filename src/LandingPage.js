const toggleStudentPurpose = (purposeId) => {
  ...
  // Existing handlers (keeping your original implementation)
  ...
  // NEW: Enhanced submit handler with purpose control
  ...
  // Safe function to get conversations array (keeping your existing implementation)
  ...
  // Safe function to get conversation content (keeping your existing implementation)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* API Key Input Section (keeping your existing implementation) */}
        {showApiKeyInput && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Left Column - Presets (keeping your existing preset layout) */}
          ...
          {/* Middle Column - Enhanced Configuration */}
          <div className="lg:col-span-1">
            <form onSubmit={handleSubmit} className="space-y-6">
              
              {/* Basic Configuration (keeping your existing basic config) */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {/* NEW: Educational Purpose Control */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="flex items-start">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-2 text-xs text-gray-500">
                              <div><strong>Tutor:</strong> {template.tutorPurposes.join(', ')}</div>
                              <div><strong>Student:</strong> {template.studentPurposes.join(', ')}</div>
                            </div>
                          </div>
                        </div>
                      </div>
                        ...
                        {tutorPurposes.concat(customPurposes.tutor).map((purpose) => (
                          <label key={purpose.id} className="flex items-start p-2 border border-gray-100 rounded hover:bg-gray-50 cursor-pointer">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {purpose.category && (
                                <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded mt-1 inline-block">
                                  {purpose.category}
                                </span>
                              )}
                            </div>
              ...
              {/* AI Settings (keeping your existing AI settings) */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {/* Advanced Settings Toggle (keeping your existing advanced settings) */}
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <button
                  type="button"
                      ...
                      {conversationData.metadata && (
                        <div className="mb-3 p-2 bg-gray-50 rounded text-xs text-gray-600">
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            {/* NEW: Show purpose control information */}
                            {conversationData.metadata.purpose_control_mode && (
                              <>
                                <span className="col-span-2">
                                  Purpose Mode: {conversationData.metadata.purpose_control_mode}
                                </span>
                              </>
                            )}
                            {conversationData.metadata.techniques_used && (
                              <span className="col-span-2">
                                Techniques: {conversationData.metadata.techniques_used.join(', ')}
                              </span>
                            )}
                            {conversationData.metadata.prompt_version && (
                              <span className="col-span-2">
                                Engine: {conversationData.metadata.prompt_version}
                              </span>
                            )}
                          </div>
                        </div>
...
const LandingPage = ({ onGenerate, isGenerating, progress, currentConversation, conversations, error, onDownload, onCopy }) => {
  ...
  // NEW: Purpose taxonomy with user-friendly descriptions
  const tutorPurposes = [
    {
      id: 'scaffolding',
  ...
  // NEW: Predefined templates for common scenarios
  ...
  // Your existing presets (keeping them as-is for compatibility)
  const presets = [
    {
      id: 'math_basic',
  ...
  // NEW: Auto-suggest purposes based on current configuration
  ...
  // NEW: Purpose selection handlers
